...
def main(page: ft.Page):
    ...
    def confirmar_devolucion(e):
        ...
        if not alquiler_actual:
            resultado.value = "⚠️ Primero verifique"
            resultado.color = "red"
            resultado.update()
            return
        
        saldo = alquiler_actual["saldo_pendiente"]
        try:
            pago_saldo_v = float(pago_saldo.value) if pago_saldo.value else 0
        except:
            pago_saldo_v = 0
        
        if saldo > 0 and pago_saldo_v < saldo:
            resultado.value = f"⚠️ Debe pagar saldo: C${saldo:.2f}"
            resultado.color = "red"
            resultado.update()
            return
        ...
        total = alquiler_actual["monto_pagado"] + pago_saldo_v + retraso + perdida
        
        supabase.table("alquileres").update({
            "fecha_devolucion_real": date.today().isoformat(),
            "monto_pagado": alquiler_actual["monto_pagado"] + pago_saldo_v,
            "saldo_pendiente": 0,
    ...
    scroll_content = ft.Column(
        controls=[
            ...
            ft.Card(ft.Container(ft.Column([
                ft.Text("🎭 REGISTRAR ALQUILER", size=18, weight="bold"),
                cedula_alq, disfraz,
                ft.Row([costo, pago, dias]),
                ft.ElevatedButton("Registrar Alquiler", on_click=registrar_alquiler)
            ], spacing=15), padding=20), elevation=3),
            ft.Card(ft.Container(ft.Column([
                ft.Text("📅 REGISTRAR RESERVA", size=18, weight="bold"),
                cedula_res, disfraz_res,
                ft.Row([costo_res, anticipo_res]),
                ft.Row([fecha_retiro_res, dias_res]),
                ft.ElevatedButton("Registrar Reserva", on_click=registrar_reserva)
            ], spacing=15), padding=20), elevation=3),
            ft.Card(ft.Container(ft.Column([
                ft.Text("📋 GESTIÓN DE RESERVAS", size=18, weight="bold"),
                ft.Row([reserva_id, pago_retiro]),
                ft.ElevatedButton("Retirar Reserva", on_click=retirar_reserva),
                ft.ElevatedButton("Ver Reservas Activas", on_click=ver_reservas_activas),
                ft.Row([reserva_cancelar]),
                ft.ElevatedButton("Cancelar Reserva", on_click=cancelar_reserva)
            ], spacing=15), padding=20), elevation=3),
            ft.Card(ft.Container(ft.Column([
                ft.Text("🔄 REGISTRAR DEVOLUCIÓN", size=18, weight="bold"),
                cedula_dev,
                ft.ElevatedButton("Verificar Alquiler", on_click=verificar_alquiler),
                pago_saldo,
                ft.Row([multa_retraso, multa_perdida]),
                ft.ElevatedButton("Confirmar Devolución", on_click=confirmar_devolucion)
            ], spacing=15), padding=20), elevation=3),
            ft.Card(ft.Container(ft.Column([
                ft.Text("📊 CONSULTAS", size=18, weight="bold"),
                ft.Row([
